...
import { writeDeterministicTarGzip } from "./deterministicTar.ts";
      ...
      yield* Effect.tryPromise({
        try: () =>
          writeDeterministicTarGzip({
            sourceDirectory: stagedPackageDir,
            entryNames: ["dist", "package.json"],
            outputPath: tarballPath,
          }),
        catch: (cause) =>
          new CliError({ message: `Failed to produce server tarball: ${tarballPath}`, cause }),
      });
